    ...
    def write(currencies, output)
      ...
      File.open output, 'w' do |file|
        ...
        currencies.each { |currency|
          next if currency[:alpha_code] == 'XTS'
          next if currency[:alpha_code] == 'XXX'
          unless registered.include?(currency[:alpha_code])
            file.write %(Currency.register english_name: '#{currency[:english_name]}', alpha_code: '#{currency[:alpha_code]}', numeric_code: #{currency[:numeric_code].to_i}\n)
            registered << currency[:alpha_code]
          end
        }
      end
    end
