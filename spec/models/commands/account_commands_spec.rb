...
describe Application::Commands::AccountCommands do
  ...
  describe described_class::RenameAccount do
    it "should initialize the command from params" do
      subject = described_class.new id: 'a-100', name: 'New account 100'
      expect(subject.id).to eql('a-100')
      expect(subject.name).to eql('New account 100')
      expect(subject.valid?).to be_truthy
    end
    
    it "should validate presence of all attributes" do
      subject = described_class.new id: nil, name: nil
      expect(subject.valid?).to be_falsey
  ...
  describe described_class::SetAccountUnit do
    it "should initialize the command from params" do
      subject = described_class.new id: 'a-100', unit: 'oz'
      expect(subject.id).to eql('a-100')
      expect(subject.unit).to eql('oz')
      expect(subject.valid?).to be_truthy
    end
    
    it "should validate presence id" do
      subject = described_class.new id: nil, unit: 'oz'
      expect(subject.valid?).to be_falsey
      expect(subject.errors[:id]).to eql ["can't be blank"]
    end
  end
