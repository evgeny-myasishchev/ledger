...
RSpec.describe Api::DevicesController, type: :controller do
	describe "routes", :type => :routing do
		...
		it 'routes PUT reset-secret-key' do
			expect({put: 'api/devices/100/reset-secret-key'}).to route_to controller: 'api/devices', action: 'reset_secret_key', id: '100'
		end
	...
	describe 'PUT reset_secret_key' do
		it 'should reset secret key of the user device' do
			expect(controller.current_user).to receive(:reset_device_secret).with("100")
			post :reset_secret_key, id: 100
			expect(response.status).to eql 200
		end
	end
